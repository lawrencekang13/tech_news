...
export default class CoreBuilder {
    ...
    getDefaultHandlerFileFilter(hasAPIRoutes, pageManifest) {
        return (file) => {
            ...
            const isNotExcludedJSFile = hasAPIRoutes ||
                ...
                this.isSSRJSFile(pageManifest, pathToPosix(path.relative(path.join(this.serverlessDir, "pages"), file)) // important: make sure to use posix path to generate forward-slash path across both posix/windows
                );
    ...
    async copyChunks(handlerDir) {
        return (await fse.pathExists(join(this.serverlessDir, "chunks")))
            ? fse.copy(join(this.serverlessDir, "chunks"), join(this.outputDir, handlerDir, "chunks"))
            : Promise.resolve();
    }
    /**
    ...
    async copyJSFiles(handlerDir) {
        await Promise.all([
            (await fse.pathExists(join(this.serverlessDir, "webpack-api-runtime.js")))
                ? fse.copy(join(this.serverlessDir, "webpack-api-runtime.js"), join(this.outputDir, handlerDir, "webpack-api-runtime.js"))
                : Promise.resolve(),
            (await fse.pathExists(join(this.serverlessDir, "webpack-runtime.js")))
                ? fse.copy(join(this.serverlessDir, "webpack-runtime.js"), join(this.outputDir, handlerDir, "webpack-runtime.js"))
                : Promise.resolve()
        ]);
    }
    ...
    async buildStaticAssets(pageManifest, routesManifest, ignorePatterns) {
        ...
        // Copy BUILD_ID file
        const copyBuildId = copyIfExists(path.join(dotNextDirectory, "BUILD_ID"), path.join(assetOutputDirectory, withBasePath("BUILD_ID")));
        const buildStaticFiles = await readDirectoryFiles(path.join(dotNextDirectory, "static"), ignorePatterns);
